...
export const writePlantRecord = `
INSERT INTO PLANT_RECORD (user_id, plant_id, content, created_at, updated_at) 
VALUES (?, (SELECT plant_id FROM PLANT WHERE user_id = ? AND plant_nickname = ?), ?, ?, ?);
`;
...
export const insertPlant = 'INSERT INTO PLANT (user_id, plant_name, plant_nickname, created_at, plant_img) VALUES (?, ?, ?, ?, ?);';
...
export const deletePlant = 'DELETE FROM PLANT '
    + 'WHERE user_id = ? AND plant_nickname = ?;';
// 식물 이미지 URL 조회
export const selectPlantImage = 'SELECT plant_img FROM PLANT WHERE user_id = ? AND plant_nickname = ?;';
...
export const getPlantInfo = 'SELECT plant_id, IFNULL(plant_img, NULL) as plant_img '
    + 'FROM PLANT '
    + 'WHERE user_id = ? AND plant_nickname = ?;';
export const insertDeadPlant = 'INSERT INTO PLANT_DEAD (user_id, plant_id, reson, memorial_letter, created_at, plant_img) '
    + 'VALUES (?, ?, ?, ?, ?, ?);';
...
export const updatePlantDetails = (updates) => `
    UPDATE PLANT
    SET ${updates.join(', ')}
    WHERE user_id = ? AND plant_nickname = ?;
`;
...
// 식물일지 삭제
export const deletePlantRecord = 'DELETE FROM PLANT_RECORD '
    + 'WHERE user_id = ? AND plant_id = (SELECT plant_id FROM PLANT WHERE user_id = ? AND plant_nickname = ?) AND created_at = ?;';

// nicToId
// nicToId SQL 문 수정
export const nicToId = 'SELECT plant_id FROM PLANT WHERE plant_nickname = ? AND user_id = ?;';
